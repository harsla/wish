...
var _ = require("underscore"),
  ...
  isTruthy = function (n) { return n ? true : false; };
...
wish.composeContextualizedRequestHandler = function () {
  ...
  return function (request, response) {
    wish.log.debug("handling contextualized request");
    async.waterfall([function (callback) { callback(undefined, wish.createContext(request, response)); }]
      .concat(_.rest(tasks)),
      wish.generateRequestCompletionCallback(_.first(tasks),
        _.filter(_.pluck(_.rest(tasks).reverse(), "finalize"), isTruthy)));
  };
};

wish.generateRequestCompletionCallback = function (responseBuilder, finalizers) {
  return function (error, context) {
    ...
    async.waterfall(
      [function (callback) { callback(undefined, context); }].concat(finalizers),
      identity
    );
  };
};
