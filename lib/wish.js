...
wish.composeContextualizedRequestHandler = function () {
  ...
  return function (request, response) {
    ...
    // invoke the request handler function chain
    async.waterfall([function (callback) {
      ...
      function (error, context) {
        ...
        // invoke the finalizer function chain
        async.waterfall(
          [function (callback) { callback(undefined, context); }].concat(
            _.filter(_.pluck(_.rest(_arguments).reverse(), "finalize"), isTruthy)
          ),
          identity
        );
      });
